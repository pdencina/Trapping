...
export default async function DashboardPage() {
  ...
  const [{ data: billeteras }, { data: operaciones }] = await Promise.all([
    supabase.from('billeteras').select('*').eq('user_id', user!.id).is('deleted_at', null),
    supabase.from('operaciones')
      .select(`*, estatus_operaciones(nombre_estatus), cuentas_destinatarios(destinatarios(name, lastname, paises(nombre_pais)))`)
      .eq('user_id', user!.id)
      .is('deleted_at', null)
  ...
  const totalEnviado = operaciones
    ...
    .reduce((acc, o) => acc + o.monto_origen, 0) ?? 0
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/operaciones" className="text-sm text-brand-600 font-medium hover:text-brand-700">
            Ver todas →
          </Link>
        </div>
        ...
        <div className="card overflow-hidden">
          {!operaciones?.length ? (
            <div className="p-12 text-center">
              ...
              <Link href="/transferir" className="btn-primary mt-4 inline-flex">
            ...
            <div className="divide-y divide-gray-50">
              {operaciones.map(op => {
                ...
                const dest = (op.cuentas_destinatarios as any)?.destinatarios
                return (
                  <div key={op.id} className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-900 truncate">
                        ...
                        <span className="text-gray-400 ml-1">· {(dest?.paises as any)?.nombre_pais}</span>
                      </p>
